...
Predicting child grooming is an incredibly difficult accentuated by the huge class imbalance given the rarity of publicly avaiable grooming chat data and the omnipresence of non grooming chat data. As such our dataset consistent of on ~3% positive classes. Through a combination of up/downsampling we arrived at a final training data mix consistent of 25% positive grooming classes. To help combat the remaining class imbalance 
...
The model was intially designed to trained on the chat texts in addition to new features engineered from linguistic analysis within the field of child grooming. As child grooming (especially in chats) often times follows a lifecycle of stages such as (building tust, isolating the child, etc.) we found such features rather simple to manual extract through various techniques such as through Bag of Words types models. 

conducted exhaustise feature analysis a
